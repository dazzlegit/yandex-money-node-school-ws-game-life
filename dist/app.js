...
var _LifeGame = __webpack_require__(14);

var _LifeGame2 = _interopRequireDefault(_LifeGame);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_vue2.default.use(_vueMaterial2.default);
_vue2.default.material.registerTheme('black');
...
var app = new _vue2.default({
	...
	methods: {
		onUserNameSet: function onUserNameSet(name) {
			this.$emit('set_user_name', name);
		}
	}
});

window.LifeGame = _LifeGame2.default;
window.App = {
	init: function init() {
		var _this = this;

		app.$on('set_user_name', function (token) {
			_this.onToken(token);
		});
	},
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_58dcf15a_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_bustCache_App_vue__ = __webpack_require__(13);
var disposed = false
...
var Component = normalizeComponent(
  ...
  __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_58dcf15a_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_bustCache_App_vue__["a" /* default */],
  __vue_template_functional__,
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)
Component.options.__file = "src/client/App.vue"
...
if (false) {(function () {
  ...
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-58dcf15a", Component.options)
  } else {
    hotAPI.reload("data-v-58dcf15a", Component.options)
' + '  }
...
var render = function() {
  ...
  return _c(
    ...
    [
      _c(
        "md-toolbar",
        { staticClass: "md-whiteframe md-whiteframe-1dp" },
        [
...
if (false) {
  module.hot.accept()
  if (module.hot.data) {
    require("vue-hot-reload-api")      .rerender("data-v-58dcf15a", esExports)
  }
}
...
var LifeGame = function () {
	function LifeGame(userName, settings) {
	...
	_createClass(LifeGame, [{
		key: 'init',
		value: function init() {
			var appendTo = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'canvas';
		...
		value: function _changeState() {
			...
			var sizeX = state.length;

			for (var i = 0; i < sizeX; i++) {
				var sizeY = state[i].length;

				for (var j = 0; j < sizeY; j++) {

					var isActive = !!state[i][j];
					var domX = i * this.size.point;
					var domY = j * this.size.point;

					if (isActive) {
						var user = state[i][j].user;
		...
		key: '_initDOMField',
		value: function _initDOMField(appendTo, dimensions) {
			var _this = this;

			this._domField = new _Canvas2.default(appendTo, this.size.point, dimensions);
			this._domField.initField();

			this._domField._canvas.addEventListener('life__mouse_click', function (event) {
				var _normalizeCoords2 = _this._normalizeCoords(event.detail),
				    x = _normalizeCoords2.x,
				    y = _normalizeCoords2.y;

				_this._render(_this._user, x, y);
				_this._send(x, y);
			});
		}
	}, {
		key: '_send',
		value: function _send(x, y) {
			var user = this._user;
			var data = {
				user: user,
				affectedPoints: [{ x: x, y: y }]
			};

			this.send(data);
		}
	}, {
		key: '_normalizeCoords',
		value: function _normalizeCoords(coords) {
			var x = coords.x,
			    y = coords.y;

			var point = this.size.point;

			var cx = Math.floor(x / point);
			var cy = Math.floor(y / point);
			return { x: cx, y: cy };
		}
	}, {
		key: '_isNew',
		value: function _isNew(x, y) {
			return !this._state[x][y];
		}
	}, {
		key: '_render',
		value: function _render(user, x, y) {
			var domX = x * this.size.point;
			var domY = y * this.size.point;
...
var Canvas = function () {
	function Canvas(appendTo, pointSize, dimensions) {
		...
		var clickFn = this._mouseClick.bind(this);
	...
	_createClass(Canvas, [{
		...
		value: function initField() {
			var lineSize = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0.5;
			var lineColor = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '#000';
			...
			try {
				for (var _iterator = 'xy'[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
		...
		key: '_mouseClick',
		value: function _mouseClick(event) {
			var _getClickCoordsFromEv = this._getClickCoordsFromEvent(event),
			    _getClickCoordsFromEv2 = _slicedToArray(_getClickCoordsFromEv, 2),
			    x = _getClickCoordsFromEv2[0],
			    y = _getClickCoordsFromEv2[1];

			this._trigger('_canvas', 'life__mouse_click', { x: x, y: y });
		}
	}, {
		key: '_trigger',
		value: function _trigger(ctx, eventName, data) {
		...
		key: '_getClickCoordsFromEvent',
		value: function _getClickCoordsFromEvent(event) {
		...
		key: 'delBlock',
		value: function delBlock(x, y) {
			this._context.clearRect(x, y, this._pointSize, this._pointSize);
		}
	}, {
		key: '_drawLine',
		value: function _drawLine(x1, y1, x2, y2, linewidth, strokestyle) {
